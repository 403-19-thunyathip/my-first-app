import streamlit as st

st.markdown("# :red[🏋️คำนวณค่าดัชนีมวลกาย BMI]")
st.write("กรอกข้อมูลน้ำหนักและส่วนสูงของคุณ เพื่อเช็กสุขภาพเบื้องต้น")

# แก้จุดที่ 1: เปลี่ยนชื่อตัวแปรให้สะกดถูกต้อง (weight)
weight = st.number_input("กรอกน้ำหนักของคุณ (กิโลกรัม):", min_value=1.0)
height_cm = st.number_input("กรอกส่วนสูงของคุณ (เซนติเมตร):", min_value=1.0)

if st.button("คำนวณค่า BMI 🎯"):
    height_m = height_cm / 100
    # แก้จุดที่ 2: ใช้ bmi (ตัวพิมพ์เล็ก) ให้เป็นมาตรฐานเดียวกันทั้งหมด
    bmi = weight / (height_m ** 2)

    st.write("---")
    st.header(f"ค่า BMI ของคุณคือ: **{bmi:.2f}**")

    # แก้จุดที่ 3: ขยับบล็อกการเช็กเงื่อนไขเข้ามาไว้ด้านใน if st.button
    if bmi < 18.5:
        st.warning("⚠️ คุณมีน้ำหนักน้อยกว่าเกณฑ์ (ผอม)")
    elif 18.5 <= bmi < 23.0:
        st.success("🎉 คุณมีน้ำหนักอยู่ในเกณฑ์ปกติ (สุขภาพดี)")
    elif 23.0 <= bmi < 25.0:
        st.info("💡 คุณเริ่มมีน้ำหนักเกินเกณฑ์ (ท้วม)")
    else:
        st.error("🚨 คุณอยู่ในเกณฑ์อ้วน ควรระวังเรื่องสุขภาพและออกกำลังกาย")

st.divider()
st.write("นางสาวธัญทิพ เชาวน์รุ่งเมธี เลขที่ 19 ม.4/3")
